...
export function NavUser({ user }: { user: Models.User<Models.Preferences> }) {
          ...
          <DropdownMenuTrigger asChild>
            <SidebarMenuButton
              ...
              <Avatar className="h-8 w-8 rounded-lg bg-foreground">
                <AvatarImage
                  src={userToDisplay.data?.data[0].avatarUrl}
                  alt={userToDisplay.data?.data[0].name}
                />
              ...
              <div className="grid flex-1 text-left text-sm leading-tight">
                <span className="truncate font-semibold">
                  {userToDisplay.data?.data[0].name}
                </span>
                <span className="truncate text-xs">
                  {userToDisplay.data?.data[0].email}
                </span>
              </div>
